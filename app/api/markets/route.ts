...
export async function GET() {
  const rows = await Promise.all(symbols.map(async (symbol) => {
    ...
    return { symbol, price, previous, changePercent: price && previous ? ((price - previous) / previous) * 100 : null, currency: meta?.currency ?? 'USD' }
  }))
  return NextResponse.json({ source: 'Yahoo Finance chart endpoint', quotes: rows })
}
